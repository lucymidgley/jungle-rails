...
RSpec.feature "User can log in", type: :feature, js: true do
  ...
  xscenario "They can log in to their account" do
    # ACT
    visit root_path
    click_on('Login')
    sleep(1)
  ...
  scenario "They cannot log in with incorrect details" do
    # ACT
    visit root_path
    click_on('Login')
    sleep(1)
    within 'form' do
      fill_in id: 'email', with: 'lucymidgley@email.com'
      fill_in id: 'password', with: 'notpassword'
      click_on 'Submit'
    end
    ...
    # VERIFY
    expect(page).to have_text 'Incorrect details!'
    save_screenshot
    puts page.html
  end
end
